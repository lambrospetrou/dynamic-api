...
For convenience during manual testing, every app also exposes a single auto-rotating "test token". Unlike the long-lived tokens, it is stored in recoverable (plaintext) form and is always visible to the owner in the dashboard and API, so they can click an app's URL without minting and managing a separate secret. It rotates roughly every 30 minutes and authenticates a private app exactly like a normal token. Exposing it in plaintext is acceptable only because it is readable solely through the Access-protected management plane, and because its short lifetime limits the impact if it is ever shared.
